...
export function CallsView() {
  ...
  Object.entries(messages).forEach(([chatId, msgList]) => {
    ...
    msgList.forEach((msg) => {
      if (msg.type === "CALL_LOG") {
        ...
        const duration =
          typeof callData.duration === "number" ? callData.duration : 0;
        const isUnanswered =
          duration === 0 ||
          callData.action === "MISSED" ||
          callData.action === "NO_ANSWER" ||
  ...
  const dbCalls = calls.map((c: any) => {
    ...
    const isGroup =
      c.chat?.isGroup || (c.participants && c.participants.length > 2);
    ...
    const joinedParticipantIds =
      c.joinedParticipantIds ||
      (c.participants
        ? c.participants
            .filter(
              (p: any) => p.hasJoined || p.status === "JOINED" || p.joinedAt,
            )
  ...
  if (filter === "missed") {
    combinedCalls = combinedCalls.filter(
      (c) => !c.isOutgoing && c.isUnanswered,
    );
  ...
  const selectedChat = selectedCall ? chats.find(c => c.id === selectedCall.chatId) : null;
              ...
              >
                <div className="flex items-center space-x-3.5 min-w-0">
                  ...
                  <div
                    ...
                    title={
                      isGroup
                        ? "Click to view group call participant details"
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center space-x-1.5 text-xs mt-0.5">
                      {isOutgoing ? (
                        ...
                        <ArrowDownLeft
                          size={15}
                          className="text-[#f15c6d] shrink-0"
                        />
                      ) : (
                        <ArrowDownLeft
                          size={15}
